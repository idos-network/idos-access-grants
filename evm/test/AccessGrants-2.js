  ...
  describe("Events", function () {
    describe("when a grant inserted", function () {
      it("Creates GrantAdded event", async function () {
        ...
        // At this moment we have only one event log fro owner and grantee from deployAndPopulateContractFixture()
        // It reflects await accessGrants.connect(signer2).insertGrant(signer3, "2A", NO_TIMELOCK);
        logs = await accessGrants.queryFilter([
          eventTopic,
          ethers.zeroPadValue(owner.address, 32),
          ethers.zeroPadValue(grantee.address, 32),
          ethers.keccak256(ethers.toUtf8Bytes("2A")),
        ])
        expect(logs.length).to.equal(1);
        expect(logs[0].data).to.equal(0n);

        const { accessGrantsV2 } = await upgradeToV2(accessGrants);
        const res = await accessGrantsV2.connect(owner).insertGrant(grantee, dataId, lockedUntil);

        // Now another event is added. But we can't parse logs generated by different ABI by some one ABI. It will fail.
        // If we try to parse 2 logs with old one ABI, then parsing the second log will fail. An vice versa.
        // So we have to get only logs reflected to same ABI and work with them. That is why we use blockFrom filter here.
        logs = await accessGrantsV2.queryFilter([
          eventTopic,
          ethers.zeroPadValue(owner.address, 32),
          ethers.zeroPadValue(grantee.address, 32),
        ], res["blockNumber"])
        expect(logs.length).to.equal(1);
        expect(logs[0].topics.length).to.equal(3);
        expect(logs[0].args[2]).to.equal(dataId);
        expect(logs[0].args[3]).to.equal(lockedUntil);
      });
    });
